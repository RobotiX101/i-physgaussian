...
wp.init()
wp.config.verify_cuda = False  # PATCHED
...
if __name__ == "__main__":
    ...
    if args.implicit:
        sys.path.insert(0, "/root/autodl-tmp/PhysGaussian")
        from implicit_mpm_solver import ImplicitMPMSolver
        mpm_solver = ImplicitMPMSolver(
            n_particles=10,
            n_grid=material_params["n_grid"],
            grid_lim=material_params["grid_lim"],
        )
        print(f"[IMPLICIT] Using ImplicitMPMSolver, dt_multiplier={args.dt_multiplier}")
    else:
    ...
    for frame in tqdm(range(frame_num)):
        ...
        if args.implicit:
            ...
            actual_steps = max(1, int(round(step_per_frame / args.dt_multiplier)))
            ...
            actual_dt_ex = substep_dt * args.dt_multiplier
            actual_steps_ex = max(1, int(round(step_per_frame / args.dt_multiplier)))
            for step in range(actual_steps_ex):
